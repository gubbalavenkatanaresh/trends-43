...
class App extends Component {
  ...
  addCartItem = product => {
    ...
    const alreadyAddItem = cartList.some(eachItem => eachItem.id === product.id)
    if (alreadyAddItem) {
  ...
  decrementCartItemQuantity = itemId => {
    const {cartList} = this.state
    const getItem = cartList.filter(eachItem => eachItem.id === itemId)
    console.log(getItem)
    console.log(getItem[0].quantity)
